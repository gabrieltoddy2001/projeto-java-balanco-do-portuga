...
// Scroll estilizado para evitar fundo branco
JScrollPane scroll = new JScrollPane(tabela);

// deixa o fundo do viewport escuro
...
// adiciona no painel (só depois disso ele tem um parent)
raiz.add(scroll, BorderLayout.CENTER);
        ...
        lblResumo.setBorder(BorderFactory.createEmptyBorder(0, 10, 0, 0));
        ...
        JPanel direita = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 5));
        direita.setBackground(fundoEscuro);
    ...
    private void montarUICliente(JPanel raiz) {
        ...
        btnSalvar.addActionListener(e -> reservaController.salvarReserva(this));
...
// Scroll estilizado (remove fundo branco)
...
hist.add(scrollHist, BorderLayout.CENTER);
        ...
        reservaController.carregarVeiculos(this);
    ...
    private void verificarDisponibilidade() {
        try {
            ...
            if (v == null) { JOptionPane.showMessageDialog(this, "Selecione um veículo."); return; }

            LocalDate ini = parseData(campoDataInicio.getText());
            LocalDate fim = parseData(campoDataFim.getText());
            if (ini == null || fim == null) {
                JOptionPane.showMessageDialog(this, "Datas inválidas."); return;
            }

            boolean conflito = new com.balancodoportuga.dao.ReservaDAO()
                    .verificarConflitoReserva(v.getId(), ini, fim);
