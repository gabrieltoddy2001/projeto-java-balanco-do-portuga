...
public class ClienteDAO {
    ...
    public void insert(Cliente c) throws SQLException {
        ...
        String sql = "INSERT INTO clients(nome, cpf, email, telefone, endereco, numero, bairro, cidade, estado) " +
                     "VALUES (?,?,?,?,?,?,?,?,?)";
        try (Connection conn = DB.getConnection();
    ...
    public void update(Cliente c) throws SQLException {
        String sql = "UPDATE clients SET nome=?, cpf=?, email=?, telefone=?, endereco=?, numero=?, bairro=?, cidade=?, estado=? WHERE id=?";
        try (Connection conn = DB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, c.getNome());
            ps.setString(2, c.getCpf());
    ...
    // Verifica se o cliente tem qualquer reserva associada
private boolean clienteTemReserva(int clienteId) throws SQLException {
    ...
    try (Connection conn = DB.getConnection();
        ...
        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return rs.getInt(1) > 0; // Se tiver qualquer reserva, retorna true
            }
        }
    }
    return false;
